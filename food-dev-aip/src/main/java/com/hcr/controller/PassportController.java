...
public class PassportController extends BaseController{
    ...
    private void synchShopcartData( HttpServletRequest request,
         ...
         *                  如果cookie中的购物车不为空，此时直接放入reids中
         * 2.redis中有数据，如果cookie中的购物车为空，那么直接把redis的购物车覆盖本地cookie
         *                  如果cookie中的某个商品在redis中存在，则以cookie为主，删除redis中的数据，把cookie中的商品直接覆盖redis中（参考京东）
         * 3.同步redis中去了以后，覆盖本地cookie购物车的数据，保证本地购物车的数据是同步最新的
         */
        //从redis中获取购物车
    ...
    public JSONResult logout(@RequestParam String userId,
        ...
        // 用户退出登陆，清除redis中的user会话信息
        redisOperator.del(REDIS_USER_TOKEN + ":" + userId);
        //分布式会话中需要清除用户数据,用户退出需要清空购物车

...
2、 BO与业务相关 VO：前端发送请求传入的数据给后端，内部传出去由后端传给前端，拿到数据展示在显示层（表现层view）

3、 搜索商品列表\通过分类id搜索商品列表 -》 功能类似，接口分开写有助于后期用户量增大减少查询压力

4、 商城购物车：可用Cookie与Redis交互，用户无登录时使用Cookie，登录后同时使用cookie与redis
    购物车存储形式 - Cookie
    无须登录、无线查库、保存在浏览器端
    优点：性能好、访问快、没有和数据库交互
    缺点1：换电脑购物车数据回丢失
    ...
    缺点2：只能存在与当前会话，不适用集群与分布式系统（不会同步到其他集群或系统中  类似单机httpSession）
...
6、创建订单预防买超单体下synchronized虽然可以使用但是性能低下且在集群下无用，不同服务器下线程仍然可以抢占同一商品
 ...
 公网即可访问，如用手机流量访问本地接口，若访问成功则隧道打通
